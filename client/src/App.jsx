...
function App() {
  ...
  const [race, setRace] = useState('READY')
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setRuntime({ state: 'LIVE', slot: slot.toLocaleString(), deployed: account?.value?.executable ? 'YES' : 'NO', relay: relay ? 'READY' : 'OFFLINE', error: '' })
    } catch (error) {
  ...
  const runRace = () => {
    setRace('20 / 20 · 1 WINNER')
    record({ label: 'Budget race', detail: 'twenty private sessions', result: 'Passed', tone: 'success' })
  }
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom"><div className="sidebar-label">TRUST BOUNDARY</div><div className="runtime-card"><div className="runtime-top"><span className="pulse" /> TEE DEVNET <span className="runtime-live">{runtime.state}</span></div><span className="runtime-url">{rpcUrl}</span><div className="runtime-meta"><span>Base slot</span><strong>{runtime.slot}</strong></div></div><small className="operator-note">No wallet, policy secret, or outcome authority lives here.</small></div>
      </aside>
      ...
      <main className="main-content">
        ...
        <div className="page-wrap">
          <section className="page-heading"><div><div className="eyebrow"><span className="eyebrow-line" />PRIVATE BUDGET / DEVNET EVIDENCE</div><h1>Capabilities stay<br /><em>on a short leash.</em></h1><p className="heading-copy">Private policy and agent ledgers run in one PER. This cockpit reads public health and shows reproducible gates; it never reads secret policy state.</p></div><div className="heading-actions"><span className={`status-pill ${runtime.state === 'LIVE' ? 'success' : 'warning'}`}><span className="status-dot" />{runtime.state}</span><button className="primary-button" onClick={runRace}>Race: {race}</button></div></section>

          <section className="metric-grid" aria-label="LEASH metrics">
            <div className="metric-card"><div className="metric-label">TEE RUNTIME</div><strong className="accent-text">{runtime.state}</strong><div className="metric-foot">{runtime.error || 'Public health only'}</div></div>
            ...
            <div className="metric-card"><div className="metric-label">TRANSPORT</div><strong className="accent-text">{runtime.relay}</strong><div className="metric-foot">signed payloads only</div></div>
          </section>
