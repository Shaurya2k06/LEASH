...
async function rpc(method, params = []) {
  ...
  if (!response.ok || body.error) {
    throw new Error(
      body.error?.message || `RPC request failed (${response.status})`,
    );
  }
  return body.result;
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (window.location.hash)
      requestAnimationFrame(() => scrollToTarget(window.location.hash, -80));
  }, []);
  ...
  const refresh = useCallback(async () => {
    ...
    const [slotResult, accountResult, relayResult, evidenceResult] =
      await Promise.allSettled([
        ...
        fetch("/evidence.json", { cache: "no-store" }).then(
          async (response) => {
            if (!response.ok)
              throw new Error(`Evidence manifest failed (${response.status})`);
    ...
    setRuntime({
      state:
        coreErrors.length === 0 && account?.value?.executable
          ? "READY"
          : "DEGRADED",
    ...
    if (evidenceResult.status === "fulfilled") {
      ...
      if (manifest.program?.id !== programId) {
        setEvidence(null);
        setEvidenceError(
          "Evidence manifest program does not match VITE_PROGRAM_ID.",
        );
      } else {
